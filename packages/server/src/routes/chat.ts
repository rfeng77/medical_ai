...
chatRouter.post("/stream", async (req: Request<unknown, unknown, ChatRequest>, res: Response) => {
  ...
  try {
    ...
    if (!participantId || !caseId || !condition || !sessionId || !message) {
      writeSse(res, "error", { error: "participantId, caseId, condition, sessionId, and message are required." });
      res.end();
      return;
    }
    ...
    const pipelineResult = await runSelfTriageTurn({
      ...
      memory,
      caseId,
      onResponseToken: (chunk) => writeSse(res, "delta", { text: chunk })
    });
